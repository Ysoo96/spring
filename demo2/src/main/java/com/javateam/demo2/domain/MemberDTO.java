...
import com.javateam.demo2.domain.MemberDTO;
...
public class MemberDTO {
	...
	// DTO => MultiValueMap
	public static MultiValueMap<String, String> toMap(MemberDTO memberDTO) throws NoSuchMethodException,
		...
		for (Field fld : fields) {
			...
			if (fld.getName().equals("log") == false) {
				...
				if (fld.getName().equals("birthday") || fld.getName().equals("joindate")) {
					...
					map.put(fld.getName(),
							Arrays.asList(new SimpleDateFormat("yyyy-MM-dd").format(method.invoke(memberDTO))));
